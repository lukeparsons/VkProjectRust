...
impl<'a, const SIZE: usize> Extensions<'a, SIZE> {
    ...
    pub fn are_in<T: ExtensionNames>(&self, available_extensions: Vec<T>) -> Result<(), VkError> {
        ...
        {
            return Err((String::from("Did not find validation layer(s) ")
                + not_found_layers.as_str())
            .into());
        }
...
impl ExtensionNames for vk::LayerProperties {
    fn get_name(&self) -> &CStr {
        self.layer_name_as_c_str()
            .expect("No NUL byte in enumerated layer")
    }
}

impl ExtensionNames for vk::ExtensionProperties {
    fn get_name(&self) -> &CStr {
        self.extension_name_as_c_str()
            .expect("No NUL byte in enumerated layer")
    }
}
